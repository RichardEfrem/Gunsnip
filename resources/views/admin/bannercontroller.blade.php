...
<section class="mt-4">
    ...
    <h1 class="text-4xl font-bold">Homepage Banner Controller</h1>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-6">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 border border-gray-300"> 
                ...
                @foreach ($banners as $item)
                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                        <td class="px-6 py-4 border border-gray-300">
                            <img src="{{ asset($item->imagepath) }}" alt="{{ $item->imagepath }}" class="object-cover rounded">
                        </td>
                        <td class="px-6 py-4 border border-gray-300">
                            <div class="flex items-center">
                                ...
                                <form id="delete-form-{{ $item->id }}" action="{{ route('deletebanner', $item->id) }}" method="POST" style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
            ...
            @else
                <tr>
                    <td colspan="3" class="text-center px-6 py-4">No Banner</td>
                </tr>
            @endif
            </tbody>
        </table>
        ...
        <div class="bg-white rounded-lg w-1/3 p-6">
            ...
            <form action="{{ route('banner.add') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex justify-end space-x-4">
                    ...
                    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg">Add Banner</button>
                </div>
            </form>
        </div>
    </div>
